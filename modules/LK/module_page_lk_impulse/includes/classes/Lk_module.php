...
namespace app\modules\module_page_lk_impulse\includes\classes;
...
class Lk_module{
	...
	public function LkAddDiscord($post){
		...
		if(empty($post['webhoock_url_offon']))
			...
			if(empty($post['webhoock_url']))
				$this->message('Введите URL Вебхука!','error');
			$auth = 1;
		}
	...
	public function LkDeleteGateway($post){
		...
		$this->message('Касса успешно удалена!','success');
	}
	...
	protected function LKvalidateGatewayData($gateway, $post){
		switch ($gateway) {
			...
			case 'paypal':
				if(empty($post['shopid']))
					$this->message('Введите E-mail бизнес аккаунта в системе PayPal ','error');
				$this->name = 'PayPal';
			break;
			case 'qiwi':
				if(empty($post['secret1']))
	...
	public function LkUpdateBalance($post){
		...
		$this->message('Новый баланс для '.$post['user'].' сохранен!','success');
	}
